...
pub fn verify_char(input: &[u8], ch: u8, inposp: &mut usize) -> Result<(), Error> {
    if *inposp >= input.len() {
        Err(InnerError::JsonBad("Too Short or Missing Fields", *inposp).into())
    } else if input[*inposp] == ch {
        *inposp += 1;
        Ok(())
    } else {
        Err(InnerError::JsonBadCharacter(input[*inposp] as char, *inposp, ch as char).into())
    }
}
...
pub(crate) fn next_object_field(input: &[u8], inposp: &mut usize) -> Result<bool, Error> {
    ...
    if *inposp >= input.len() {
        return Err(InnerError::JsonBad("Too short", *inposp).into());
    }
    if input[*inposp] == b'}' {
        *inposp += 1;
        Ok(true)
        ...
        *inposp += 1;
        ...
        Err(InnerError::JsonBad("Unexpected char", *inposp).into())
    }
}
...
pub(crate) fn read_id(input: &[u8], inposp: &mut usize, output: &mut [u8]) -> Result<(), Error> {
    ...
    if *inposp + 64 >= input.len() {
        return Err(InnerError::JsonBad("Too short reading id", *inposp).into());
    }
...
pub(crate) fn read_pubkey(
    ...
    verify_char(input, b'"', inposp)?;
    if *inposp + 64 >= input.len() {
        return Err(InnerError::JsonBad("Too short reading pubkey", *inposp).into());
    }
...
pub(crate) fn read_u64(input: &[u8], inposp: &mut usize) -> Result<u64, Error> {
    ...
    if !any {
        return Err(InnerError::JsonBad("Expected a positive integer", *inposp).into());
    }
    Ok(value)
}
...
pub(crate) fn read_kind(input: &[u8], inposp: &mut usize) -> Result<u16, Error> {
    ...
    if value > 65535 {
        Err(InnerError::JsonBad("Kind larger than 65535", *inposp).into())
    } else {
        Ok(value as u16)
    }
}
...
pub(crate) fn read_tags_array(
    ...
    if output.len() < 4 {
        return Err(InnerError::BufferTooSmall(4).into());
    }
    ...
    if output.len() < outpos {
        return Err(InnerError::BufferTooSmall(outpos).into());
    }
    ...
    loop {
        ...
        match input[*inposp] {
            ...
            _ => return Err(InnerError::JsonBad("Tag array bad character", *inposp).into()),
        }
    }
...
pub(crate) fn count_tags(input: &[u8], mut inpos: usize) -> Result<usize, Error> {
    ...
    match input[inpos] {
        ...
        b'[' => (),           // expected
        _ => return Err(InnerError::JsonBad("Tag array bad initial character", inpos).into()),
    }
    ...
    loop {
        match input[inpos] {
            ...
            _ => return Err(InnerError::JsonBad("Tag array bad character", inpos).into()),
        }
    }
}
...
pub(crate) fn read_sig(input: &[u8], inposp: &mut usize, output: &mut [u8]) -> Result<(), Error> {
    ...
    if *inposp + 128 >= input.len() {
        return Err(InnerError::JsonBad("Too short reading sig", *inposp).into());
    }
...
pub(crate) fn burn_string(input: &[u8], inposp: &mut usize) -> Result<(), Error> {
    ...
    if input[*inposp] == b'"' {
        *inposp += 1;
        ...
        Err(InnerError::JsonBad("Unterminated string", *inposp).into())
    }
}
    ...
    match input[*inposp] {
        ...
        _ => {
            if b"123456789".contains(&input[*inposp]) {
                ...
                return Err(
                    InnerError::JsonBad("Too short burning an unused JSON value", *inposp).into(),
                );
            }
        }
    }
...
pub(crate) fn burn_null(input: &[u8], inposp: &mut usize) -> Result<(), Error> {
    if *inposp + 4 <= input.len() && &input[*inposp..*inposp + 4] == b"null" {
        *inposp += 4;
        ...
        Err(InnerError::JsonBad("Expected null", *inposp).into())
    }
}
...
pub(crate) fn burn_true(input: &[u8], inposp: &mut usize) -> Result<(), Error> {
    if *inposp + 4 <= input.len() && &input[*inposp..*inposp + 4] == b"true" {
        *inposp += 4;
        ...
        Err(InnerError::JsonBad("Expected true", *inposp).into())
    }
}
...
pub(crate) fn burn_false(input: &[u8], inposp: &mut usize) -> Result<(), Error> {
    if *inposp + 5 <= input.len() && &input[*inposp..*inposp + 5] == b"false" {
        *inposp += 5;
        ...
        Err(InnerError::JsonBad("Expected false", *inposp).into())
    }
}
