...
pub fn next_code_point(input: &[u8]) -> Result<Option<(u32, usize)>, Error> {
    ...
    if len < 2 {
        return Err(InnerError::Utf8Error.into());
    }
    let y = input[1];
...
pub fn encode_utf8(code: u32, dst: &mut [u8]) -> Result<usize, Error> {
    ...
    let len = unsafe {
        if code < MAX_ONE_B {
            if dst.is_empty() {
                return Err(InnerError::BufferTooSmall(1).into());
            } else {
            ...
            if dst.len() < 2 {
                return Err(InnerError::BufferTooSmall(2).into());
            } else {
            ...
            if dst.len() < 3 {
                return Err(InnerError::BufferTooSmall(3).into());
            } else {
