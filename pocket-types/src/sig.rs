...
/// A nostr event signature
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, AsRef, Deref, From, Into)]
pub struct Sig([u8; 64]);

impl Sig {
    /// Create from an internal representation slice of bytes
    pub fn from_bytes(bytes: [u8; 64]) -> Sig {
        Sig(bytes)
    }
    ...
    /// Internal representation a slice of bytes
...
impl TryInto<Sig> for &[u8] {
    type Error = Error;

    fn try_into(self) -> Result<Sig, Self::Error> {
        let array: [u8; 64] = self.try_into()?;
        Ok(Sig(array))
    }
}
