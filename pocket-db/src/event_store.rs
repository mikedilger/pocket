...
impl EventStore {
    ...
    pub(crate) fn store_event(&self, event: &Event) -> Result<usize, Error> {
        ...
        loop {
            let result = self.event_map.append(event_size, |dst| {
                event.copy(dst).map_err(std::io::Error::other)
            });
...
impl<'a> Iterator for EventStoreIter<'a> {
    type Item = Result<&'a Event, Error>;
...
mod tests {
    ...
    fn test_event_store() {
        ...
        if let Ok(event) = unsafe { store.get_event_by_offset(offset1) } {
            assert_eq!(event, event1);
        } else {
            panic!("EVENT 1 IS WRONG");
        }

        if let Ok(event) = unsafe { store.get_event_by_offset(offset2) } {
            assert_eq!(event, event2);
        } else {
            panic!("EVENT 2 IS WRONG");
        }

        if let Ok(event) = unsafe { store.get_event_by_offset(offset3) } {
